...
function AddTeamModal() {
  const { addTeam, setFormData, formData, resetForm } = useTeams();
  ...
  const handleSubmit = async (e) => {
    ...
    await addTeam(e, updatedFormData);
    ...
    document.getElementById("team_modal").close(); // Close modal
  };
      ...
      <dialog id="team_modal" className="modal">
        <div className="modal-box w-11/12 max-w-3xl">
          ...
          <div className="modal-action">
            <button
              className="btn"
              onClick={() => document.getElementById("team_modal").close()}
            >
              Close
            </button>
          </div>
