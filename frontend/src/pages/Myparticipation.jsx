...
const Myparticipation = () => {
  const { myparticipation, loading, error, fetchEvents, fetchmyparticipation } =
    useEvents();

  const user = JSON.parse(localStorage.getItem("user")); // Get user object
  const user_id = user ? user.user_id : null;

  // Fetch participation only when user_id is available
  useEffect(() => {
    if (user_id) {
      fetchmyparticipation(user_id);
    }
  }, [user_id]);
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      ...
      {myparticipation.length === 0 && !loading && (
